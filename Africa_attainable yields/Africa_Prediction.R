...
data_pred <- read_csv("LongLatAfrica_Light(v3).csv")
names(data_pred)
...
data_pred$planting_date <- as.Date(
  sprintf("%04d-%02d-01", data_pred$Growing_year, data_pred$planting_month)
)
...
radiation <- predictBL(model_lat, data$lat)^2
...
data_pred2 <- data_pred[which(data_pred$min_value<=0),]
data_pred2 <- data_pred2 %>% filter(!is.na(long),!is.na(lat),!is.na(min_value))
pts2 <- st_as_sf(data_pred2, coords = c("long", "lat"), crs = 4326)

# limiting yield-----------------

ggplot() +
  geom_sf(data = Africa, fill = "grey95", color = "grey60", linewidth = 0.3) +
  geom_sf(data = pts2, aes(color = min_value), size = 2, alpha = 0.9) +
