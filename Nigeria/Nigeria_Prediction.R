...
Growing_year <- rep(c(2021), each=n)
...
data_pred <- data_pred %>%
  mutate(
    ...
    vwc_10kpa = {
      cols <- dplyr::select(., dplyr::matches("^vw10_(0_5|5_15|15_30|30_60|60_100)$"))
      rowSums(cols * weights_0_100, na.rm = TRUE) / total_depth
    },
    vwc_33kpa = {
...
#This removes locations with soil NAs 
data_pred <- data_pred[!is.na(data_pred$clay_0_5_pct), ]
...
Clay_p=predictBL(model_clay,log(data_pred$clay))
Clay_min=predictBL(model_clay,2.7)
Clay_min

Clay_p=pmax(Clay_p,Clay_min)
Clay_p2=Clay_p^2
...
Sand_p=predictBL(model_sand,log(data_pred$sand))
Sand_min=predictBL(model_sand,3.6)

Sand_p=pmax(Sand_p,Sand_min)
Sand_p2=Sand_p^2
...
pH_p=predictBL(model_pH,data_pred$soil_pH)
pH_min=predictBL(model_pH,5)

pH_p=pmax(pH_p,pH_min)
pH_p2=pH_p^2
...
SOC_p=predictBL(model_SOC,log(data_pred$SOC))
SOC_min=predictBL(model_SOC,3.65)

SOC_p=pmax(SOC_p,SOC_min)
SOC_p2=SOC_p^2
...
BD_p=predictBL(model_BD,data_pred$BD/1000)
BD_min=predictBL(model_BD,1.57)

BD_p=pmax(BD_p,BD_min)
BD_p2=BD_p^2
...
data_pred$PAW <- data_pred$vwc_33kpa - data_pred$vwc_1500kpa # Calculates plant available water (PAW)

PAW_p=predictBL(model_paw,data_pred$PAW/1000)
PAW_min=predictBL(model_paw,0.205)

PAW_p=pmax(PAW_p,PAW_min)
PAW_p2=PAW_p^2
...
ggplot() +
  geom_sf(data = Africa, fill = "grey95", color = "grey60", linewidth = 0.3) +
  geom_sf(data = pts, aes(color = min_value), size = 1.75, alpha = 0.9) +
...
#    high = "green",
  ...
  labs(
    title = "Predicted Yieldby the Law of the Minimum: Soil and climate factors",
    x = "Longitude",
    y = "Latitude"
  )
